...
class Features:
    ...
    def _get_names_types_converted_columns(self):
        ...
        columns.append(CC.StatusDecisionOfObservation)
    ...
    def _add_lenghts_comments_source_data(self):
        self.all_features.append('lenght_all_texts')
        
        column_names = map(SC._Column.get_name, SC.all_comment)
        for column_name in column_names:
            new_column_name = 'lenght_' + column_name
            self.all_features.append(new_column_name)
    
    def _add_quantity_of_patients(self):
        self.all_features.append('quantity_patients_admission_to_hospital')
        self.all_features.append('quantity_patients_departure_from_hospital')
...
class PreparedData:
    ...
    def _combine_converted_data_and_air_pollution(self, converted_data, air_pollution): 
        return pd.merge(converted_data, air_pollution, how='left', left_index=True, right_index=True)
...
#             data = pd.concat([data, pd.DataFrame(frequency_of_words)], axis=1)
    ...
    def _count_lenghts_text_columns(self, data):
        data['lenght_all_texts'] = 0
        column_names = list(map(SC._Column.get_name, SC.all_comment))
        for column_name in column_names:
            new_column_name = 'lenght_' + column_name
            data[new_column_name] = 0
            
            for index_row in range(len(data)):
                text = data.loc[index_row, column_name]
                if text:
                    data.loc[index_row, new_column_name] = len(text)
                    data.loc[index_row, 'lenght_all_texts'] += len(text)
        return data
    ...
    def _count_quantity_of_patients(self, data):
        ...
        data_admission = get_array_for_hospital_and_date(name_admission, name_zeros='quantity_patients_departure_from_hospital', name_ones='quantity_patients_admission_to_hospital')
        data_departure = get_array_for_hospital_and_date(name_departure, name_zeros='quantity_patients_admission_to_hospital', name_ones='quantity_patients_departure_from_hospital')
        data_balance = pd.concat([data_admission, data_departure], ignore_index=True)
        
        data_balance = data_balance.sort_values(by=['Date'])
        grouped = data_balance.groupby(data_balance[name_of_hospital])
        grouped_data = grouped.cumsum()
        
        data_balance = data_balance.drop(['quantity_patients_admission_to_hospital', 'quantity_patients_departure_from_hospital'], axis=1)
        data_balance = pd.merge(data_balance, grouped_data, how='left', left_index=True, right_index=True)
        ...
        data_balance['quantity_patients_present_in_hospital'] = data_balance['quantity_patients_admission_to_hospital'] - data_balance['quantity_patients_departure_from_hospital']
        
        grouped_balance = data_balance['quantity_patients_present_in_hospital'].groupby(level=name_of_hospital)
        ...
        for feature in ['quantity_patients_admission_to_hospital', 'quantity_patients_departure_from_hospital', 'quantity_patients_present_in_hospital']:
            data_moment[feature] /= data_moment['max_balance']
            data_moment[feature][empty_mask] = -1
            
            data_feature = pd.Series(data_moment[feature], name=feature, dtype='float32')
    ...
    def _count_rounded_features(self, data):
        ...
        for feature_name_with_postfix in all_features:
            ...
            coordinates[np.isnan(coordinates)] = -2
    ...
    def _centred_geo_coordinates(self, data):
        ...
        for feature, center in zip(['Longitude', 'Latitude'], Settings.CENTER_OF_MOSCOW):
            coordinate = data[feature].to_numpy() - center
            coordinate[np.isnan(coordinate)] = -1
...
class LearningData:
    ...
    def _get_all_excluded_features(self, excluded_features, severity_of_disease):
        if severity_of_disease == 'Умер':
            additional_name = [CC.ResultOfHospitalization.get_name()]
            excluded_features = list(set(excluded_features) | set(additional_name))
